...
def _load_multimnist_pickle(path: str) -> Any:
    """Load legacy multi_mnist pickles without NumPy 2.4+ dtype(align=...) deprecation noise."""
    try:
        from numpy.exceptions import VisibleDeprecationWarning
    except ImportError:
        VisibleDeprecationWarning = getattr(  # type: ignore[misc]
...
class MultiMNISTDataset(VisionDataset):    
    ...
    def __init__(
        ...
        if download:
            ...
            if dataset == 'mnist':
                trainX, trainLabel, testX, testLabel = _load_multimnist_pickle(
                    f'{self.root}/data/multi_mnist.pickle'
                )

            # MultiFashionMNIST: multi_fashion.pickle
            if dataset == 'fashion':
                trainX, trainLabel, testX, testLabel = _load_multimnist_pickle(
                    f'{self.root}/data/multi_fashion.pickle'
                )

            # Multi-(Fashion+MNIST): multi_fashion_and_mnist.pickle
            if dataset == 'fashion_and_mnist':
                trainX, trainLabel, testX, testLabel = _load_multimnist_pickle(
                    f'{self.root}/data/multi_fashion_and_mnist.pickle'
                )

            if self.train:
                self.data = torch.from_numpy(trainX.reshape(120000, 1, 36, 36)).float()
                self.targets = torch.from_numpy(trainLabel).long()
            else:
                self.data = torch.from_numpy(testX.reshape(20000, 1, 36, 36)).float()
                self.targets = torch.from_numpy(testLabel).long()
    ...
    def download(self):
        ...
        if self.dataset == 'mnist':
        ...
        elif self.dataset == 'fashion_and_mnist':
            mnist_train_1 = torchvision.datasets.MNIST(
                root=self.root,
                download=True,
                train=True,
                transform=transforms.Compose([transforms.ToTensor()]),
            ...
            mnist_test_1 = torchvision.datasets.MNIST(
                root=self.root,
                download=True,
                train=False,
                transform=transforms.Compose([transforms.ToTensor()]),
            )
            mnist_train_2 = torchvision.datasets.FashionMNIST(
                root=self.root,
                download=True,
                train=True,
                transform=transforms.Compose([transforms.ToTensor()]),
            )
            mnist_test_2 = torchvision.datasets.FashionMNIST(
                root=self.root,
                download=True,
                train=False,
                transform=transforms.Compose([transforms.ToTensor()]),
            )
            mnist_train = torch.cat((mnist_train_1, mnist_train_2), dim=0)
            mnist_test = torch.cat((mnist_test_1, mnist_test_2), dim=0)
...
def create_new_sample(left_idx, right_idx, dataset):
    """Creates new samples for the MultiMNIST dataset. The procedure is the following: two random MNIST samples with
...
class MultiMNISTDataset3Digits(VisionDataset):    
    ...
    def __init__(
        ...
        if download:
            ...
            raise NotImplementedError("Download is not supported for MultiMNIST3Digits")
